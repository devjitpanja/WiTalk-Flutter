    ...
    return Scaffold(
      ...
      body: async.when(
        ...
        data: (comms) => ListView.builder(itemCount: comms.length, itemBuilder: (_, i) {
          final c = comms[i] as Map<String, dynamic>; final name = c['name'] ?? ''; final pic = c['image'] as String?; final members = c['member_count'] ?? 0; final id = c['id'] ?? ''; final isLive = c['active_adda_room_id'] != null || c['is_live'] == true;
          return ListTile(leading: isLive ? LiveGroupAvatar(picture: pic, size: 48, primaryColor: AppColors.primary, onPress: () => context.push('/community-adda-list/$id')) : CircleAvatar(radius: 24, backgroundColor: AppColors.border, backgroundImage: pic != null ? CachedNetworkImageProvider(pic) : null, child: pic == null ? Text((name.isNotEmpty ? name[0] : '?').toUpperCase(), style: const TextStyle(color: Colors.white)) : null),
